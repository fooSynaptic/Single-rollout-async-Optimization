...
def grpo_loss_fn(
    ...
    # Explicit scalar aliases for Phase2 compare reports / repro-todo.
    _pg = stat["loss"]
    if isinstance(_pg, torch.Tensor) and _pg.numel() > 0:
        stats_tracker.scalar(pg_loss=float(_pg.detach().float().mean().item()))
    ...
    if "filtered_fraction" in stat:
        _mask_ratio = float(stat["filtered_fraction"])
        stats_tracker.scalar(
            rs_filtered_fraction=_mask_ratio,
            **{"dis/mask_ratio": _mask_ratio},
        )
